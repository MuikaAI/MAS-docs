...
**排查**：先在 INFO 日志中查找 `[Event] user_message`，再查看 DEBUG 中的模型等待与失败原因。行动任务在后台执行，期间可以继续聊天。
...
- 先让 Muika 按来源核对原话、日期和相邻内容。
- 明确说明修正；新素材会在下一次日记整理时更新事实。可用 `.reflect` 手动整理。
- memory 工具的 forget 操作可撤下事实；原始对话仍保留。

## 性能

### Core 内存占用过高

- 检查模型的 `context_window` 是否符合实际服务窗口。
- 原文保存在 SQLite，工作上下文会按预算压缩。检查摘要模型错误日志。
